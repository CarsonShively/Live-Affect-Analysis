...
class LowLatencyModel(tf.keras.Model):
    def __init__(self):
        ...
        self.category_spatial = tf.keras.layers.DepthwiseConv2D(filters=64, kernel_size=3, padding="same", use_bias=False)
        ...
        self.valence_spatial = tf.keras.layers.DepthwiseConv2D(filters=64, kernel_size=3, padding="same", use_bias=False)
        ...
        self.arousal_spatial = tf.keras.layers.DepthwiseConv2D(filters=64, kernel_size=3, padding="same", use_bias=False)
        ...
        self.dominace_projection_norm = tf.keras.layers.GroupNormalization(groups=8)
        self.dominance_spatial = tf.keras.layers.DepthwiseConv2D(filters=64, kernel_size=3, padding="same", use_bias=False)
        ...
        self.gender_spatial = tf.keras.layers.DepthwiseConv2D(filters=64, kernel_size=3, padding="same", use_bias=False)
        ...
        self.age_spatial = tf.keras.layers.DepthwiseConv2D(filters=64, kernel_size=3, padding="same", use_bias=False)
        ...
        self.category_norm = tf.keras.layers.GroupNormalization()
        self.valence_norm = tf.keras.layers.GroupNormalization()
        self.arousal_norm = tf.keras.layers.GroupNormalization()
        self.dominance_norm = tf.keras.layers.GroupNormalization()
        self.gender_norm = tf.keras.layers.GroupNormalization()
        self.age_norm = tf.keras.layers.GroupNormalization()
        ...
        self.category_norm2 = tf.keras.layers.GroupNormalization()
        self.valence_norm2 = tf.keras.layers.GroupNormalization()
        self.arousal_norm2 = tf.keras.layers.GroupNormalization()
        self.dominance_norm2 = tf.keras.layers.GroupNormalization()
        self.gender_norm2 = tf.keras.layers.GroupNormalization()
        self.age_norm2 = tf.keras.layers.GroupNormalization()
    ...
    def call(self, x, training=False):
        ...
        category = tf.keras.activations(category)
        ...
        category = self.category_norm2(category, training=training)
        valence = self.valence_norm2(valence, training=training)
        arousal = self.arousal_norm2(arousal, training=training)
        dominance = self.dominance_norm2(dominance, training=training)
        gender = self.gender_norm2(gender, training=training)
        age = self.age_norm2(age, training=training)
