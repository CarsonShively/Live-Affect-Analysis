...
class GatedFeatureFusion(tf.keras.Model):
    def __init__(self):
        ...
        self.satisfaction_gate = tf.keras.layers.Dense(128, activation="sigmoid")
        self.calmness_gate = tf.keras.layers.Dense(128, activation="sigmoid")
        ...
        self.satisfaction_hidden = tf.keras.layers.Dense(128)
        self.calmness_hidden = tf.keras.layers.Dense(128)
        ...
        self.satisfaction_norm = tf.keras.layers.BatchNormalization()
        self.calmness_norm = tf.keras.layers.BatchNormalization()
    ...
    def call(self, x, training=False):
        ...
        satisfaction = self.satisfaction_gate(gate_input)
        calmness = self.calmness_gate(gate_input)
        ...
        satisfaction = satisfaction * f_prime + (1 - satisfaction) * c_prime
        calmness = calmness * f_prime + (1 - calmness) * c_prime
        ...
        satisfaction = self.satisfaction_hidden(satisfaction)
        calmness = self.calmness_hidden(calmness)
        ...
        satisfaction = tf.keras.activations.gelu(satisfaction)
        calmness = tf.keras.activations.gelu(calmness)
        ...
        satisfaction = self.dropout(satisfaction, training=training)
        ...
        satisfaction = self.satisfaction_head(satisfaction)
